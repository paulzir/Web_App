...
if (isset($_GET['page'])) {
	...
	$searchitem=$_GET['searchitem'];
		...
		$sql="SELECT COUNT(orderid) FROM orders,customers,products,categories".
			" WHERE $searchtype LIKE '%$searchitem%' AND orders.customer=customers.customerid".
			" AND orders.product=products.prod_id AND orders.category=categories.cat_id ";
		...
		$query="SELECT orderid,category,product,price,lastname,date,payment,details,prod_name,cat_name".
			" FROM orders,customers,products,categories".
			" WHERE $searchtype LIKE '%$searchitem%' AND orders.customer=customers.customerid".
			" AND orders.product=products.prod_id AND orders.category=categories.cat_id ".			
			" ORDER BY orderid DESC LIMIT $offset, $rec_limit";
		 ...
		 <div id="orders">
            	...
            	<!-- Paging Orders --> 
					<h4>	
						...
						{
						$last = $pages - 2;
						echo "<a href='.?page=$submit&searchtype=$searchtype&searchitem=$searchitem&pages=$last'>Previous</a> | ";
						echo "<a href='.?page=$submit&searchtype=$searchtype&searchitem=$searchitem&pages=$pages'>Next</a>";
						}
						else if( $pages == 0 )
						{
						echo "<a href='.?page=$submit&searchtype=$searchtype&searchitem=$searchitem&pages=$pages'>Next</a>";
			  ...
			  <!-- /Paging Orders -->
                ...
                <?php 
				 ...
				 ?>
					...
					<tr><td colspan="2" align="left"><input name="delete_order" type="submit" value="Delete" id="delete"></td><td align="left">
						<input name="update_order" type="submit" value="Update" id="update"></td></tr>
					</table>
					</form>              
					</li>
                <?php } ?>
